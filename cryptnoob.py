...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 17 11:31:00 2018
...
import crycompare, crycompare.history,crycompare.price
...
#        histo = h.histo_day(coin,'USD',all_data=True)
#        print(i, coin)
        if  coin == 'XRP':
            histo = h.histo_day(coin,'USD',all_data=True)
...
sub_coins = [sorted_length[i][0] for i in range(1)]

df_histo['Date']=df_histo.index
df_histo['Date'] = pd.to_datetime(df_histo['Date'])
#plt.figure(figsize = (18,9))
#plt.plot(range(df_histo.shape[0]),df_histo['close'])
#plt.xticks(range(0,df_histo.shape[0],500),df_histo['Date'].loc[::500],rotation=45)
#plt.xlabel('Date',fontsize=18)
#plt.ylabel('Mid Price',fontsize=18)
#plt.show()

high_prices = df_histo.loc[:,'high'].values
low_prices = df_histo.loc[:,'low'].values
mid_prices = (high_prices+low_prices)/2.0

ltrain = int(0.75*len(mid_prices))
train_data = mid_prices[:ltrain]
test_data = mid_prices[ltrain:]
...
test_points_seq = np.arange(len(train_data),int(len(train_data)*1.1),30).tolist()
...
best_prediction_epoch = 98 # replace this with the epoch that you got the best results when running the plotting code

plt.figure(figsize = (18,18))
plt.subplot(2,1,1)
plt.plot(range(df_histo.shape[0]),all_mid_data,color='b')
...
plt.xlabel('Date',fontsize=18)
plt.ylabel('Mid Price',fontsize=18)
plt.xlim(len(train_data),len(train_data)+200)
...
#
#
##kaggle.KaggleApi.download_file('sudalairajkumar/cryptocurrencypricehistory',response="ripple_price",outfile="../kagfile.csv") 
#fname='xrp.csv'
#sub = 'kaggle datasets download --unzip -d lalo52000/cryptocurrency-dataset -p ./datacrypto    -f '+fname
##subprocess.run(sub, shell=True)
## ====================== Loading Data from Kaggle ==================================
## You will be using HP's data. Feel free to experiment with other data.
## But while doing so, be careful to have a large enough dataset and also pay attention to the data normalization
#df = pd.read_csv(os.path.join('datacrypto', fname),delimiter=',',usecols=['time','open','high','low','volumefrom','volumeto'], parse_dates=['time'])#,'Volume','Market Cap'])
